...
func memo(w http.ResponseWriter, r *http.Request) {
	...
	s := "<html>" +
		...
		// 削除ボタンを追加（別のフォームとして定義するか、JSで遷移させる）
		"<input type='button' value='削除' onclick=\"location.href='/mdelete'\" style='margin-left:10px;' />" +
		"</form>" +
		"</body></html>"
	w.Write([]byte(s))
}
...
func mdelete(w http.ResponseWriter, r *http.Request) {
	// ファイルに初期テキストを書き込む（またはファイルを削除する）
	err := os.WriteFile(saveFile, []byte("ここにメモを記入してください。"), 0644)
	if err != nil {
		fmt.Printf("Delete error: %v", err)
	}
	fmt.Println("Reset to default text")
	// メモ画面に戻る
